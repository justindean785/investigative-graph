...
import {
  ENTITY_TYPES,
  RELATIONSHIP_TYPES as RELATIONSHIP_TYPES_OBJ,
...
const RELATIONSHIP_TYPES = RELATIONSHIP_TYPES_OBJ.map((t) => t.value);
...
const GraphView = ({
  ...
  const nodes = useMemo(() => {
    return entities.map((entity, index) => {
      const typeInfo =
        ENTITY_TYPES.find((t) => t.value === entity.kind) || ENTITY_TYPES[0];
  ...
  const handleAddRelationship = async () => {
    if (
      !newRelationship.source_entity_id ||
            ...
            <Panel position="top-center">
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-sm px-4 py-2 flex items-center gap-3">
                <span className="text-amber-400 text-sm">
                  {entities.length} entities found. Link them to discover
                  relationships.
                </span>
                <Button
                  onClick={onNavigateToEntities}
            ...
            <DialogContent className="glass-strong border-white/10 text-white max-w-md">
              ...
              <div className="space-y-4 mt-4">
                <div>
                  ...
                  <Select
                    value={newRelationship.source_entity_id}
                    onValueChange={(value) =>
                      setNewRelationship({
                        ...newRelationship,
      ...
      {selectedNode && (
        <div className="absolute right-0 top-0 bottom-0 w-80 glass-strong border-l border-white/10 p-5 overflow-y-auto z-10">
          ...
          <div className="space-y-4">
            <div>
              <p className="text-xs text-cyan-500/80 uppercase tracking-wider mb-1">
                Type
              </p>
