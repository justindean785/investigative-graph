"""
Test script for autonomous investigation engine.
"""
import requests
import json
import time
...
def main():
    ...
    # Pivot depth analysis
    if entities:
        max_depth_reached = max((e.get("metadata", {}).get("depth", 0) for e in entities), default=0)
        print(f"\nMax Pivot Depth: {max_depth_reached}")
    
    # Check for multi-level pivoting
    derived_entities = [e for e in entities if e.get("sources") and any("derived_from" in str(s) for s in e.get("sources", []))]
